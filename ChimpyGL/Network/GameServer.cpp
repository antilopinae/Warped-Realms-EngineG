...
bool GameServer::OnClientConnect(std::shared_ptr<Connection<GameMsgTypes>> client)
{
...

  {
    ...
    // } else {
    //     std::cerr << "[SERVER] CRITICAL: Failed to create Ship for client " << tentativeClientID << "! Player unusable." <<
    //     std::endl;
    // }
...
void GameServer::OnMessage(std::shared_ptr<Connection<GameMsgTypes>> client, const Message<GameMsgTypes>& message)
{
  ...
  switch (message.header.id)
  {
    ...
    default:
      std::cout << "[SERVER] Unhandled message type from client " << clientID << ": ID "
                << static_cast<uint32_t>(message.header.id) << std::endl;
      break;
  }
}
...
void GameServer::Update(size_t nMaxMessages, bool bWait)
{
  ...
  if (deltaTime <= 0.0f)
    return;    // Skip update if no time passed
