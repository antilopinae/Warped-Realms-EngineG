...
Game::Game() : mWindow(nullptr), mRenderer(nullptr), mIsRunning(true), mUpdatingActors(false), mClient(), mServer(80000) {}
...
void Game::ProcessNetworkMessages()
{
  while (!mClient.IncomingMessages().empty())
  {
    ...
    switch (message.header.id)
    {
      ...
      default: std::cout << "[CLIENT] Unhandled message type: " << static_cast<uint32_t>(message.header.id) << std::endl; break;
    }
  }
}
