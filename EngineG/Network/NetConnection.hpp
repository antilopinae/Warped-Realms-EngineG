...
public:
    ...
    Connection(Owner parent, asio::io_context& ioContext, asio::ip::tcp::socket socket, TSQueue<OwnedMessage<T>>& qIn);
...
template <typename T>
Connection<T>::Connection(Owner parent, asio::io_context& ioContext, asio::ip::tcp::socket socket, TSQueue<OwnedMessage<T>>& qIn)
    : mSocket(std::move(socket)), mIoContext(ioContext), mQMessagesIn(qIn), mOwnerType(parent) {
}
...
void Connection<T>::ConnectToServer(const asio::ip::tcp::resolver::results_type& endpoints) {
    if (mOwnerType == Owner::Client) {
        asio::async_connect(mSocket, endpoints, [this](std::error_code ec, const asio::ip::tcp::endpoint& /*endpoint*/) {
            if (!ec) {
                ReadHeader();
            } else {
...
void Connection<T>::Disconnect() {
    if (IsConnected()) {
        asio::post(mIoContext, [this]() {
            ...
            mSocket.close(ec);
            if (ec) {
                std::cerr << "[" << mId << "] Socket close error: " << ec.message() << std::endl;
            }
        });
    }
}
...
void Connection<T>::ReadHeader() {
    ...
    asio::async_read(mSocket,
                     asio::buffer(&mMsgTemporaryIn.header, sizeof(MessageHeader<T>)),
                     [this, self](std::error_code ec, std::size_t length) {
                         if (!ec) {
                             if (mMsgTemporaryIn.header.size > MAX_MESSAGE_BODY_SIZE) {
                                 std::cerr << "[" << mId << "] Read Header Error: Message size exceeds limit ("
                                           << mMsgTemporaryIn.header.size << "). Disconnecting." << std::endl;
                                 mSocket.close();
                                 return;
                             }
                             ...
                             std::cout << "[" << mId << "] Read Header Fail: " << ec.message() << ". Connection closing."
                                       << std::endl;
                             mSocket.close();
                         }
                     });
...
void Connection<T>::ReadBody() {
    auto self = this->shared_from_this();
    asio::async_read(mSocket,
                     asio::buffer(mMsgTemporaryIn.body.data(), mMsgTemporaryIn.body.size()),
                     [this, self](std::error_code ec, std::size_t length) {
                         if (!ec) {
                             AddToIncomingMessageQueue();
                         } else {
                             std::cout << "[" << mId << "] Read Body Fail: " << ec.message() << ". Connection closing."
                                       << std::endl;
                             mSocket.close();
                         }
                     });
...
void Connection<T>::WriteHeader() {
    ...
    asio::async_write(mSocket,
                      asio::buffer(&mQMessagesOut.front().header, sizeof(MessageHeader<T>)),
                      [this, self](std::error_code ec, std::size_t length) {
                          if (!ec) {
                              ...
                              std::cout << "[" << mId << "] Write Header Fail: " << ec.message() << ". Connection closing."
                                        << std::endl;
                              mSocket.close();
                          }
                      });
...
void Connection<T>::WriteBody() {
    ...
    auto self = this->shared_from_this();
    asio::async_write(mSocket,
                      asio::buffer(mQMessagesOut.front().body.data(), mQMessagesOut.front().body.size()),
                      [this, self](std::error_code ec, std::size_t length) {
                          if (!ec) {
                              mQMessagesOut.pop_front();
                              if (!mQMessagesOut.empty()) {
                              ...
                              std::cout << "[" << mId << "] Write Body Fail: " << ec.message() << ". Connection closing."
                                        << std::endl;
                              mSocket.close();
                          }
                      });
