...
GameEngine::GameEngine()
  ...
  try {
    mLinker
        .func_wrap(
            ...
            [](wasmtime::Caller caller, uint32_t ptr, uint32_t len) {
              ...
              std::any& host_data_any = store_context.get_data();
              ...
              if (!component) {
                std::cerr << "CRITICAL_WASM_LOG_ERROR: env.engine_log_wasm called with null component context after cast."
                          << std::endl;
    ...
    mLinker.func_wrap("env", "emscripten_notify_memory_growth", [](int32_t /*memory_index*/) {}).unwrap();
...
bool GameEngine::AttachComponent(const std::string& entityName, const std::string& slotName, const std::string& wasmModulePath) {
  ...
  if (entity_slots.count(slotName)) {
    ...
    LogMessage("GameEngine: Slot '" + slotName + "' on entity '" + entityName + "' is occupied by '" + old_component_id +
               "'. Unloading old component.");
    if (!UnloadComponentFromSlot(entityName, slotName)) {
      LogMessage("GameEngine ERROR: Failed to unload existing component '" + old_component_id + "' from slot '" + slotName +
                 "'. Attach failed.");
      return false;
    }
  }
  ...
  try {
    auto new_component_ptr =
        std::make_unique<WasmComponent>(new_component_id, wasmModulePath, entityName, slotName, this, mEngine);
...
bool GameEngine::UnloadComponentFromSlot(const std::string& entityName, const std::string& slotName) {
  ...
  if (slot_it == entity_slots.end()) {
    LogMessage("GameEngine INFO: UnloadComponentFromSlot - Slot '" + slotName + "' on entity '" + entityName +
               "' is already empty.");
    return true;
  }
...
void GameEngine::ProcessMessageQueue() {
  ...
  for (const auto& queued_msg : current_queue) {
    ...
    if (slot_it == entity_it->second.mComponentSlots.end()) {
      LogMessage("GameEngine WARN: Target slot '" + target_slot_name + "' on entity '" + target_entity_name +
                 "' not found. Skipping.");
      continue;
    }
    ...
    if (component_it != mLoadedComponents.end() && component_it->second) {
      ...
      LogMessage("GameEngine WARN: Target component ID '" + target_component_id +
      ...
      for (const auto& msg_payload : inboxCopy) {
        try {
          if (!component_ptr->DeliverMessage(msg_payload.mFromId, msg_payload.mContent)) {
            LogMessage("GameEngine WARN: Component '" + id + "' DeliverMessage returned false for message from '" +
                       msg_payload.mFromId + "'.");
          }
        } catch (const wasmtime::Error& e) {
...
void GameEngine::ScheduleDeferredWasmUpdate(const std::string& componentId) { m_deferredWasmUpdates.push_back({componentId}); }
...
void GameEngine::ProcessDeferredWasmUpdates() {
  ...
  for (const auto& call : updatesToProcess) {
    ...
    if (component_it != mLoadedComponents.end() && component_it->second && component_it->second->IsInitialized()) {
      ...
      LogMessage("GameEngine WARN: Deferred WASM update for component '" + call.targetComponentId +
...
void GameEngine::SendMessage(const std::string& fromComponentId, const std::string& toEntityAndSlotName,
                             const std::string& messageContent) {
  mMessageQueueInternal.push_back({fromComponentId, toEntityAndSlotName, messageContent});
}

void GameEngine::LogWasmException(const std::string& operationName, const std::exception& e, const std::string& componentId) {
  std::ostringstream errorLog;
  errorLog << "GameEngine EXCEPTION (std): Component '" << componentId << "' Operation '" << operationName << "'. "
           << "Exception: '" << e.what() << "'.";
  LogMessage(errorLog.str());
  std::cerr << errorLog.str() << std::endl;
}

void GameEngine::LogWasmException(const std::string& operationName, const wasmtime::Error& e, const std::string& componentId) {
  std::ostringstream errorLog;
  errorLog << "GameEngine EXCEPTION (wasmtime::Error): Component '" << componentId << "' Operation '" << operationName << "'. "
           << "Error: '" << e.message() << "'.";
  LogMessage(errorLog.str());
  std::cerr << errorLog.str() << std::endl;
}

void GameEngine::LogWasmException(const std::string& operationName, const wasmtime::Trap& e, const std::string& componentId) {
  std::ostringstream errorLog;
  errorLog << "GameEngine EXCEPTION (wasmtime::Trap): Component '" << componentId << "' Operation '" << operationName << "'. "
           << "Trap: '" << e.message() << "'.";
  LogMessage(errorLog.str());
  std::cerr << errorLog.str() << std::endl;
}
