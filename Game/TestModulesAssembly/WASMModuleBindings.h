...
#error "No module type defined for WASMModuleBindings. Define SHIP_MODULE, WALL_MODULE, or PLAYER_MODULE."
  ...
  emscripten::class_<EngineG::Game>("Game").smart_ptr<std::shared_ptr<EngineG::Game>>("Game")  // Add smart_ptr support
      ;
  ...
  emscripten::class_<EngineG::Actor>("Actor")
      ...
      .function("DrawTexture",
                emscripten::optional_override([](EngineG::Actor& actor, const std::string& textureName, Rect dstRect,
                                                 int drawOrder) { actor.DrawTexture(textureName, dstRect, drawOrder); }));

  emscripten::value_object<Rect>("Rect").field("x", &Rect::x).field("y", &Rect::y).field("w", &Rect::w).field("h", &Rect::h);
